...
자바의 제네릭 타입은 컴파일 시점에만 존재하고 런타임 시에는 제네릭 정보가 지워지는데 이걸 타입 이레이저라고 한다
타입 이레이저를 사용하는 이유는 제네릭은 자바의 처음부터 도입이 된게 아니기 때문에 기존 코드와 호환을 위해 제네릭을 사용했던 코드를 컴파일 시점에 원시 타입으로 변경하여 호환이 되도록 한다 

### 타입 이레이저의 역할

1. **타입 정보 제거:** 컴파일러가 제네릭 타입을 컴파일할 때, 모든 제네릭 타입 정보는 제거되고, 원시 타입(Raw Type)으로 대체된다. 예를 들어, `List<String>`은 컴파일 후에 단순히 `List`로 변환된다
    - **예시:** `List<String>`은 컴파일 후 `List`가 된다
2. **타입 안전성 보장:** 제네릭 타입 이레이저는 컴파일 시간에 타입 체크를 수행하여 타입 안전성을 보장한다. 이는 런타임에 발생할 수 있는 타입 관련 오류를 방지한다
    - **예시:** 컴파일러는 `List<String>`에 `Integer`를 추가하려고 하면 오류를 발생시킨다
3. **바이트코드 호환성 유지:** 타입 이레이저를 통해 제네릭 코드를 기존의 비제네릭 코드와 호환되도록 한다
이는 자바 5 이전에 작성된 코드와의 호환성을 유지하기 위함이다
